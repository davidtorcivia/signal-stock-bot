...
_OPEN_PROMPT = (
    ...
    "     `bot__credit` shows whether risk is actually being "
    "     repriced — credit leads equity vol, so HY OAS widening "
    "     into a calm tape is worth more than the VIX print. "
    "     `bot__rvol` compares implied to realized before you pay "
    "     for premium, and `bot__skew` / `bot__flow` show what the "
    "     options market is positioned for on a given expiry.\n"
    ...
    "     `portfolio_write_option` to SELL premium (cash-secured "
    "     puts, covered calls, or credit spreads once you own the "
    "     protective leg) and `portfolio_close_option` to buy a "
    "     short back, "
